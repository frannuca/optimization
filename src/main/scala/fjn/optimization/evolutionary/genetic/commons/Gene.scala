package fjn.optimization.evolutionary.genetic.commons
...
case class  Gene(minLevel:Double,maxLevel:Double,nBits:Int) extends Ordered[Gene] with tParameter {
   ...
   */
  def :=(d:Seq[Int])={d.copyToArray[Int](binaryCode,0,nBits)}
  ...
  def getValue():Double={
     ...
     minLevel + (listAux.foldLeft(0.0D)((acc,x)=> acc + (2 << (x._1.toInt -1)) * x._2.toDouble)) * dt;
   }
  ...
  private def toBinaryArray(d:Double):Unit = {
    reset();
    var nVal:Long=  math.ceil((d - minLevel)/dt).toLong;
